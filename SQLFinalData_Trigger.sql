...
GO
INSERT INTO Printing (Printer_ID, Student_ID, Document_ID, PrintTime)
VALUES
    ('PR0004', 'ST0002', 'D00004','2024-11-01 10:30:25');
GO
...
BEGIN
    SELECT 
        ...
        COUNT(CASE WHEN CONVERT(DATE, prnt.PrintTime) = CONVERT(DATE, GETDATE()) THEN 1 END) AS PrintsToday
    FROM 
        printing prnt
    JOIN 
        printer pr ON prnt.Printer_ID = pr.Printer_ID
    GROUP BY 
        p.Printer_ID, pr.PlaceAt, pr.IsDisable;
END;
...
as
begin
	select p.student_id, d.dname, d.pages
	from printing p, document d
	where p.Student_ID = @SID and p.Document_ID = d.Document_ID
end;
...
as
begin
	select count(*) as BCount, sum(numberpage) as BSum 
